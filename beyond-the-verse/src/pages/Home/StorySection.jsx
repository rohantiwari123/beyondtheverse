...
export default function StorySection() {
  ...
  const fetchGeminiData = async (subjectName) => {
    ...
    try {
      ...
      if (!apiKey) {
        setGeminiData("🚨 Error: API Key not found. Restart your server (npm run dev).");
      ...
      Keep it simple and around 150 words. No markdown formatting.`;

      // ✅ Switching to the most stable 'v1' endpoint
      ...
      // 🚨 Debugging: Agar abhi bhi error aaye, toh hume asli wajah dikhegi
      if (!response.ok) {
        console.error("Gemini API Error:", data);
        setGeminiData(`🚨 Google API Error: ${data.error?.message || "Unknown Error"}`);
        return;
      }
      ...
      if (data.candidates && data.candidates[0]?.content?.parts[0]?.text) {
        ...
        setGeminiData("🚨 API responded but no content was generated.");
      }
