...
const Page = React.forwardRef(({ children, number }, ref) => {
  return (
    <div className="bg-white overflow-hidden shadow-xl rounded-2xl relative border border-slate-100 flex-1 min-h-[500px] w-full max-w-[450px]" ref={ref}>
...
const ResearchForm = ({ showToast }) => {
  ...
  const handleBodyPageChange = (index, value) => {
    ...
    setFormData((prev) => ({ ...prev, bodyPages: newBodyPages }));
  };

  const addBodyPage = () => {
    setFormData((prev) => ({ ...prev, bodyPages: [...prev.bodyPages, ""] }));
  ...
  const removeBodyPage = (index) => {
    ...
    setFormData((prev) => ({ ...prev, bodyPages: newBodyPages }));
    setCurrentPage(Math.max(0, currentPage - (isMobile ? 1 : 2)));
  };
  ...
  const totalPages = 2 + formData.bodyPages.length + 1;

  const pages = [
    <Page key="meta" number={1}>
    ...
    <Page key="abstract" number={2}>
      ...
      <div className="flex-1 flex flex-col">
        <textarea
          ...
          onChange={handleInputChange}
          className="w-full flex-1 border-2 border-slate-100 rounded-xl bg-slate-50 p-4 text-sm leading-relaxed focus:border-teal-500 focus:bg-white focus:outline-none resize-none min-h-[300px]"
    ...
    <Page key="submit" number={totalPages}>
  ...
  const handleNext = () => setCurrentPage((prev) => Math.min(prev + (isMobile ? 1 : 2), totalPages - 1));
  const handlePrev = () => setCurrentPage((prev) => Math.max(0, prev - (isMobile ? 1 : 2)));
      ...
      {isExpanded && (
        <div className="border-t border-slate-100 bg-slate-50/50 py-8">
          <div className="mx-auto flex flex-col items-center">
            ...
            <div className="flex justify-center gap-6 px-4 w-full max-w-[950px]">
              {visiblePages}
            </div>
            ...
            <div className="mt-8 flex w-full max-w-[400px] justify-between items-center px-4 sm:max-w-[500px]">
              ...
              {currentPage + (isMobile ? 1 : 2) < totalPages ? (
                <button
                  type="button"
                  onClick={handleNext}
                  className="flex items-center gap-2 rounded-full px-5 py-2.5 text-[12px] font-bold text-slate-600 transition-all hover:bg-slate-100 border border-slate-200 shadow-sm bg-white min-w-[110px] justify-center"
