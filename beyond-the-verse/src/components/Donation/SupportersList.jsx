    ...
    <div className="animate-fade-in-up">
      ...
      <div className="flex p-1 mb-6 sm:mb-8 bg-slate-100/80 rounded-xl sm:rounded-2xl border border-slate-200/60 max-w-[240px] sm:max-w-[280px] shadow-inner">
        <button
          ...
          className={`flex-1 text-xs sm:text-sm font-black py-2 sm:py-2.5 rounded-lg sm:rounded-xl flex items-center justify-center gap-1.5 sm:gap-2 transition-all duration-300 ${
            activeTab === "recent"
      ...
      <div className="space-y-4 sm:space-y-5">
        {listToRender.length === 0 ? (
          <div className="text-center py-10 sm:py-12 bg-slate-50 border border-slate-100 rounded-[2rem] border-dashed">
             ...
             <p className="text-sm sm:text-base text-slate-500 font-medium">No supporters yet.</p>
              ...
              >
                ...
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    {activeTab === "top" && (
                      <div className={`font-black text-xl sm:text-2xl lg:text-3xl w-6 sm:w-8 lg:w-10 text-center shrink-0 ${rankColors[index]}`}>
                        #{index + 1}
                      </div>
                    )}

                    {/* Dynamic Avatar */}
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center font-black text-lg sm:text-xl lg:text-2xl shadow-sm border border-white/60 shrink-0 transition-transform group-hover:scale-105 ${avatarStyle}`}>
                      {firstChar}
                    </div>
                    
                    <div>
                      <div className="font-extrabold text-slate-800 text-sm sm:text-base lg:text-lg flex items-center gap-1.5 sm:gap-2">
                        {firstName}
                      ...
                      <div className="flex items-center gap-2 mt-0.5 sm:mt-1">
                        {/* Time Ago */}
                        <span className="text-[9px] sm:text-[10px] lg:text-xs text-slate-400 font-black uppercase tracking-widest">
                          {timeAgo(d.timestamp)}
                        </span>
                        ...
                        {badge.label !== "Supporter" && (
                          <>
                            ...
                            <span className={`text-[8px] sm:text-[9px] lg:text-[10px] font-black uppercase tracking-widest ${badge.color}`}>
                              {badge.label}
                            </span>
                          </>
                        )}
                  ...
                  {/* Amount Badge */}
                  <span className={`font-black text-sm sm:text-base lg:text-lg px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border shadow-sm shrink-0 transition-transform group-hover:scale-105 ${badge.label !== "Supporter" ? 'bg-white text-slate-800 border-slate-100' : 'bg-teal-50 text-teal-700 border-teal-100'}`}>
                ...
                {d.message && activeTab === "recent" && (
                  <div className="relative mt-1 sm:mt-2 bg-white/60 backdrop-blur-sm border border-white/80 rounded-xl sm:rounded-2xl p-3 sm:p-4 ml-12 sm:ml-16 lg:ml-20 text-xs sm:text-sm text-slate-600 font-medium leading-relaxed shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] z-10">
                    <i className="fa-solid fa-quote-left text-teal-500/10 absolute -left-2 -top-2 text-3xl sm:text-4xl bg-white rounded-full"></i>
                    <p className="relative z-10 italic">"{d.message}"</p>
                  </div>
                )}
              </div>
            );
