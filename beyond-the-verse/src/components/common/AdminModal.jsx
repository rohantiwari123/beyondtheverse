    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center sm:p-4 md:p-6 bg-slate-900/80 backdrop-blur-sm transition-opacity animate-fade-in">
      ...
      <div className="bg-slate-50 w-full h-[100dvh] sm:h-[90vh] sm:rounded-[2rem] lg:max-w-6xl relative shadow-2xl flex flex-col overflow-hidden border border-slate-200">
          ...
          <div className="bg-white px-4 sm:px-6 md:px-8 pt-5 sm:pt-6 md:pt-8 pb-0 border-b border-slate-200 shrink-0">
            <div className="flex justify-between items-center mb-5 sm:mb-6 pr-10 sm:pr-0">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl text-slate-800">Admin Center</h2>
                  <span className="text-[9px] sm:text-[10px] text-teal-600 mt-1 block">Beyond The Verse</span>
                </div>
              </div>
              <button onClick={handleLogout} className="hidden sm:flex bg-rose-50 hover:bg-rose-500 text-rose-600 hover:text-white text-sm px-4 py-2 rounded-xl items-center gap-2 transition-all border border-rose-100 shadow-sm active:scale-95">
            ...
            <div className="flex gap-4 sm:gap-6 lg:gap-8 overflow-x-auto no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
              {[
                ...
                <button 
                  ...
                  className={`pb-3 text-xs sm:text-sm lg:text-base border-b-[3px] transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === tab.id ? 'border-teal-500 text-teal-700' : 'border-transparent text-slate-400 hover:text-slate-600 hover:border-slate-300'}`}
                >
              ...
              <div className="space-y-4 sm:space-y-6 h-full flex flex-col animate-fade-in-up">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 shrink-0">
                  <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[10px] sm:text-xs text-slate-400">Total Raised</p>
                  ...
                  <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[10px] sm:text-xs text-slate-400">Total Donors</p>
                  ...
                  <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 sm:col-span-2 md:col-span-1">
                    ...
                    <div>
                      <p className="text-[10px] sm:text-xs text-slate-400">Goal Progress</p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between items-stretch sm:items-center bg-white p-3 sm:px-4 sm:py-3 rounded-2xl border border-slate-200 shadow-sm shrink-0">
                  ...
                  <button onClick={handleExportCsv} className="bg-slate-900 hover:bg-black text-white text-sm px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 w-full sm:w-auto shrink-0">
                  ...
                  <div className="overflow-x-auto overflow-y-auto flex-1">
                    <table className="w-full min-w-[700px] text-left border-collapse">
                      <thead className="bg-slate-50 sticky top-0 border-b border-slate-200 z-10">
                        <tr>
                          <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400">Date</th>
                          <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400">Donor Info</th>
                          <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400">UTR Ref</th>
                          <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-teal-600 text-right">Amount</th>
                          <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400 text-center">Actions</th>
                        </tr>
                      </thead>
                        ...
                        {filteredDonations.length === 0 ? (
                          ...
                          filteredDonations.map((d) => (
                            <tr key={d.id} className="hover:bg-slate-50 transition-colors group">
                              ...
                              <td className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-600 font-mono">
                              ...
                              <td className="p-3 sm:p-4 text-sm sm:text-lg text-teal-700 text-right">
            ...
            {activeTab === 'users' && (
              <div className="h-full flex flex-col animate-fade-in-up">
                ...
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6 shrink-0">
                  ...
                  <div>
                    <h3 className="text-slate-800 text-base sm:text-lg">Registered Citizens</h3>
                    <p className="text-[10px] sm:text-xs text-slate-400 mt-0.5">Total Accounts: {usersList.length}</p>
                  </div>
                </div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex-1 flex flex-col">
                  {isFetchingUsers ? (
                     <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                       ...
                       <p className="text-[10px] sm:text-xs">Scanning Database...</p>
                     </div>
                  ) : (
                    <div className="overflow-x-auto overflow-y-auto flex-1">
                      <table className="w-full min-w-[500px] text-left border-collapse">
                        <thead className="bg-slate-50 sticky top-0 border-b border-slate-200 z-10">
                          <tr>
                            <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400">Citizen Info</th>
                            <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400 text-center">Role</th>
                            <th className="p-3 sm:p-4 text-[10px] sm:text-xs text-slate-400 text-right">Unique ID</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {usersList.length === 0 ? (
                              ...
                              <tr key={user.uid} className="hover:bg-slate-50 transition-colors">
                                <td className="p-3 sm:p-4">
                                  <div className="flex items-center gap-3">
                                    <div className={`h-8 w-8 sm:h-10 sm:w-10 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0 ${user.role === 'admin' ? 'bg-slate-800' : 'bg-teal-500'}`}>
                                      {user.name?.charAt(0).toUpperCase() || "?"}
                                    </div>
                                    <div>
                                      <p className="text-slate-800 text-sm">{user.name || "Unknown"}</p>
                                      <p className="text-[10px] sm:text-[11px] text-slate-400 mt-0.5">{user.email || "No Email"}</p>
                                    </div>
                                  </div>
                                </td>
                                <td className="p-3 sm:p-4 text-center">
                                  <span className={`px-2.5 py-1 rounded-md text-[9px] sm:text-[10px] border ${
                                    user.role === 'admin' 
                                ...
                                <td className="p-3 sm:p-4 text-right">
                                  <code className="text-[9px] sm:text-[10px] bg-slate-100 text-slate-500 px-2 py-1 rounded-md border border-slate-200 font-mono">
                                    {user.uid}
                                  </code>
                                </td>
                              </tr>
            ...
            {activeTab === 'subjects' && (
              <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 animate-fade-in-up pb-10">
                ...
                <div className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
                  <h3 className="text-lg sm:text-xl text-slate-800 mb-1 sm:mb-2 flex items-center gap-2">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full border border-slate-200 bg-slate-50 rounded-xl sm:rounded-2xl py-3 sm:py-3.5 pl-11 pr-4 outline-none focus:border-teal-500 focus:bg-white focus:ring-2 focus:ring-teal-500/20 transition-all text-sm"
                    ...
                    <button
                      ...
                      className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl whitespace-nowrap disabled:bg-slate-300 disabled:text-slate-500 transition-all shadow-md active:scale-95 w-full sm:w-auto flex justify-center items-center gap-2 text-sm sm:text-base"
                    >
                ...
                <div className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
                  <h4 className="text-slate-800 mb-4 border-b border-slate-100 pb-3 flex items-center justify-between text-sm sm:text-base">
                  ...
                  {subjectsList.length === 0 ? (
                    <div className="text-center py-10 text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-sm">No topics added yet.</div>
                  ) : (
                      ...
                      {subjectsList.map(sub => (
                        <div key={sub.id} className="bg-slate-50 border border-slate-200 p-4 sm:p-5 rounded-2xl flex justify-between items-start group hover:border-teal-300 hover:shadow-md transition-all">
                          <div className="flex-1 pr-4">
                            <h5 className="text-slate-800 text-sm sm:text-base mb-1.5">{sub.name}</h5>
                            <p className="text-[10px] sm:text-xs text-slate-500 line-clamp-3">{sub.definition}</p>
                          </div>
              ...
              <div className="max-w-3xl mx-auto space-y-4 sm:space-y-6 animate-fade-in-up pb-10">
                ...
                <div className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
                  <div className="flex-1">
                    <h3 className="text-base sm:text-lg lg:text-xl text-slate-800 flex items-center gap-2 mb-1">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                    <div className="relative flex-1 sm:w-40">
                      ...
                      <input 
                        ...
                        className="w-full border border-slate-200 bg-slate-50 rounded-xl py-2.5 pl-7 pr-3 outline-none focus:border-teal-500 focus:bg-white transition-colors text-slate-800 text-sm sm:text-base"
                      />
                    </div>
                    <button onClick={handleSaveTarget} className="bg-slate-900 hover:bg-black text-white px-5 sm:px-6 rounded-xl transition-all shadow-md active:scale-95 text-sm shrink-0">Save</button>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-teal-50 to-emerald-50 p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-teal-100 shadow-sm relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <h3 className="text-base sm:text-lg lg:text-xl text-teal-900 flex items-center gap-2 mb-1">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:ml-10">
                      <div className="relative flex-1 sm:max-w-xs">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-teal-600 text-sm">₹</span>
                        <input 
                          type="number" 
                          value={scholarshipAmount} 
                          onChange={(e) => setScholarshipAmount(e.target.value)} 
                          placeholder="Min Amount (e.g. 50)" 
                          className="w-full border border-teal-200/60 bg-white rounded-xl py-3 pl-8 pr-4 outline-none focus:border-teal-500 text-teal-900 text-sm sm:text-base transition-colors"
                        />
                      </div>
                      <button onClick={handleCreateLink} className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md shadow-teal-500/20 active:scale-95 w-full sm:w-auto shrink-0 text-sm sm:text-base">
                    ...
                    {generatedLink && (
                      <div className="mt-5 sm:ml-10 p-3 sm:p-4 bg-white border border-teal-200 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 animate-fade-in-up">
                        <div className="w-full overflow-x-auto no-scrollbar">
                          <code className="text-[10px] sm:text-xs text-teal-800 font-mono bg-teal-50 px-2 sm:px-3 py-1.5 rounded-lg border border-teal-100 whitespace-nowrap block">
                            {generatedLink}
                          </code>
                        </div>
                        <span className="text-[10px] sm:text-xs text-emerald-600 whitespace-nowrap bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100 flex items-center justify-center gap-1 w-full sm:w-auto shrink-0">
        ...
        <div className="sm:hidden bg-white border-t border-slate-200 p-4 shrink-0">
           <button onClick={handleLogout} className="w-full bg-rose-50 text-rose-600 text-sm py-3 rounded-xl flex items-center justify-center gap-2 active:scale-95">
