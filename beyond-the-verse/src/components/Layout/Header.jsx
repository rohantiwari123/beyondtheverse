          ...
          <div className="hidden lg:flex items-center gap-2 xl:gap-4 shrink-0">
            {isAuthenticated && userName && (
              <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-100 px-3 py-1.5 xl:px-4 xl:py-2 rounded-full text-indigo-700 shadow-sm" title={userName}>
                ...
                <span className="text-xs xl:text-sm font-bold truncate max-w-[100px] xl:max-w-[150px] notranslate">{userName.split(' ')[0]}</span>
              </div>
            )}

            <div className="relative" ref={dropdownRef}>
              <button onClick={() => setIsLangOpen(!isLangOpen)} className="flex items-center gap-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 px-3 py-1.5 xl:px-4 xl:py-2 rounded-xl text-sm font-semibold transition-all notranslate">
                <i className="fa-solid fa-globe text-teal-600"></i>
                <span className="hidden xl:inline">{currentLang}</span>
                <span className="xl:hidden">{currentLang.slice(0, 3)}</span>
