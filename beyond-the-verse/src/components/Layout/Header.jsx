...
const styles = {
  header:
    "bg-white/95 backdrop-blur-xl fixed top-0 left-0 right-0 z-[100] border-b w-full overflow-visible transition-colors duration-300 border-slate-200",
  spacer: "h-14 sm:h-16 w-full shrink-0",
  logoContainer: "flex items-baseline gap-1",
  brandFirst:
    "text-[22px] sm:text-[26px] lg:text-[34px] font-cabinet font-black tracking-tighter leading-none text-slate-900",
  brandMiddle:
    "text-[16px] sm:text-[20px] lg:text-[24px] lowercase tracking-tighter leading-[0.85] font-serif italic font-bold text-slate-400",
  brandLast:
  ...
  actionBtnBase:
    "h-8 w-8 xl:h-9 xl:w-9 flex items-center justify-center rounded-full transition-all border",
  actionBtnActive: "bg-teal-50 text-teal-600 border-teal-200",
  actionBtnInactive:
    "border-transparent text-slate-500 hover:bg-slate-100 hover:text-slate-800",
  profileBtnBase:
    "flex items-center justify-center w-8 h-8 xl:w-9 xl:h-9 rounded-full transition-all border",
  ...
  logoutBtn:
    "bg-rose-50 text-rose-600 border border-rose-100 px-3 py-1.5 rounded-lg text-[11px] xl:text-xs hover:bg-rose-100 transition-all ml-1",
  loginBtn:
    "bg-teal-600 text-white px-4 py-1.5 rounded-lg text-[11px] xl:text-xs hover:bg-teal-700 transition-all shadow-sm ml-1",
  notifDropdown:
    "absolute right-0 mt-2 w-[280px] sm:w-[320px] bg-white border rounded-2xl shadow-xl z-50 overflow-hidden animate-fade-in origin-top-right border-slate-200",
  notifHeader:
    "flex items-center justify-between px-4 py-3.5 border-b bg-slate-50/80 border-slate-100",
  notifItemUnread:
    "p-4 border-b cursor-pointer transition-colors flex gap-3.5 border-slate-50 bg-teal-50/40 hover:bg-slate-50",
  notifItemRead:
  ...
  mobileCloseBtn:
    "h-8 w-8 flex items-center justify-center bg-slate-50 text-slate-400 rounded-full",
  mobileLinkBase:
  ...
  mobileBottomContainer:
    "p-4 pb-8 border-t flex flex-col gap-3 border-slate-100 bg-slate-50",
  ...
  mobileLogoutBtn:
    "w-full bg-white text-rose-600 py-2.5 rounded-xl text-xs border active:bg-rose-50 transition-colors shadow-sm font-bold uppercase tracking-wider border-rose-100",
  mobileLoginBtn:
    "w-full bg-teal-600 text-white py-2.5 rounded-xl text-center text-xs font-bold uppercase tracking-wider shadow-lg shadow-teal-500/20",
};

export default function Header() {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isSearchExpanded, setIsSearchExpanded] = useState(false); // 🌟 New Search State
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-start min-w-max lg:w-1/4">
            ...
            >
              ...
              <span className={styles.brandTagline}>
                <div className="h-px w-3 bg-teal-500/40"></div> Empowering
                Education
              </span>
            </Link>
          </div>
                  ...
                  <>
                    ...
                    <div className={styles.notifDropdown}>
                      <div className={styles.notifHeader}>
                        <span className="text-slate-800 text-[13px] uppercase font-bold">
                          Notifications
                        </span>
                        {unreadCount > 0 && (
                          <span className="bg-teal-100 text-teal-700 text-[9px] font-bold uppercase px-2 py-1 rounded-md border border-teal-200">
                            {unreadCount} New
                          </span>
                        )}
                      </div>
                          ...
                          <>
                            ...
                            {unreadNotifs.map((notif) => (
                              <div
                                ...
                                className={styles.notifItemUnread}
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-[13px] truncate text-slate-800 font-medium">
                                    {notif.title}
                                  </p>
                            ...
                            {readNotifs.map((notif) => (
                              <div
                                key={notif.id}
                                onClick={() => handleNotificationClick(notif)}
                                className={styles.notifItemRead}
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-[13px] truncate text-slate-600">
                                    {notif.title}
                                  </p>
                                  <p className="text-[11px] text-slate-500 line-clamp-2 mt-1">
                                    {notif.message}
            ...
            <div className="hidden lg:flex items-center gap-1.5 xl:gap-2">
              {isAuthenticated && userName && (
                <>
                  ...
                  <Link
                    to="/profile"
                    className={`${styles.profileBtnBase} ${
                      isPathActive("/profile")
                        ? styles.profileBtnActive
                        : styles.profileBtnInactive
                    }`}
                    title="Profile"
                  >
                    {userProfilePic ? (
                      ...
                      <div
                        className={`w-full h-full rounded-full flex items-center justify-center text-[11px] xl:text-xs font-bold ${
                          isPathActive("/profile")
                            ? styles.profileFallbackActive
                            : styles.profileFallbackInactive
                        }`}
                      >
      ...
      <div className={`md:hidden fixed top-14 left-0 right-0 z-[90] bg-white border-b border-slate-100 shadow-xl transition-all duration-300 ease-in-out transform ${isSearchExpanded ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="p-3">
          <GlobalSearch isMobileExpanded={isSearchExpanded} onSelect={() => setIsSearchExpanded(false)} />
        </div>
      </div>
      ...
      >
        <div className={styles.mobileHeader}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-800 uppercase font-bold text-sm tracking-tight">
              Menu
            </span>
          </div>
          <button
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1">
          ...
          {canAccessAdmin && (
            <Link
              to="/admin"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`${styles.mobileLinkBase} mt-2 ${
                isPathActive("/admin")
                  ? styles.mobileAdminActive
                  : styles.mobileAdminInactive
              }`}
            >
              <div className="w-5 flex justify-center shrink-0">
                <i
          ...
          {isAuthenticated && (
            <div
              className={`p-2 rounded-xl border flex items-center justify-between transition-colors ${
                isPathActive("/profile") || isPathActive("/settings")
                  ? styles.mobileProfileCardActive
                  : styles.mobileProfileCardInactive
              }`}
            >
              <Link
                to="/profile"
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex items-center gap-3 flex-1 overflow-hidden px-1"
              >
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-base shrink-0 border overflow-hidden ${
                    isPathActive("/profile")
                      ? styles.mobileProfileImgActive
                      : styles.mobileProfileImgInactive
                  }`}
                >
                  {userProfilePic ? (
                    <img
              ...
              <div className="flex items-center gap-1 border-l border-slate-100 pl-1">
                <Link
                  ...
                  className={`w-9 h-9 flex items-center justify-center rounded-lg transition-colors ${
                    isPathActive("/settings")
                      ? styles.mobileSettingsBtnActive
                      : styles.mobileSettingsBtnInactive
                  }`}
                >
